...
class News1Controller extends Controller {
    ...
    public function doAdd2(){   
        ...
        if(!$info) {
            ...
            $mediannewsModel = D("mediannews"); //创建模型
            $data = $mediannewsModel->create(); //组织数据
            if(!$data){
                exit($mediannewsModel->getError());
            }else{
                //设置img字段属性(目录+名字)
     ...
     public function edit1($id){
        if(isset($_POST['submit'])){
            ...
            $mediannewsModel = D("mediannews");
            $news = $mediannewsModel->where("id='%d'",$id)->find(); //从数据库找到需要修改的用户信息
            $this->assign("news",$news);
            $this->display();
        }
    }
